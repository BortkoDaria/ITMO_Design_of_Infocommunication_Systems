...
namespace lab4
{
    ...
    class TaxiServices : TransportService
    {
        ...
        public override string ToString()
        {
            string s = String.Format("{0}, поездка категории {1}",
            Name, Category);
            return s;
        }
    }
    class Shipping : TransportService
    {
        ...
        public override string ToString()
        {
            string s = String.Format("{0}, доставка по тарифу {1}",
            Name, Tariff);
            return s;
        }
    }

    class TaxiForChildren : TransportService
    {
        public int ChildrenSeats { get; set; }
        public TaxiForChildren(string name, int seats) :
        base(name)
        {
            ChildrenSeats = seats;
        }
        public override double CostTransportation(double distance)
        {
            return distance * (1 + ChildrenSeats / 2);
        }
        public override string ToString()
        {
            string s = String.Format("{0}, поездка c {1} детским(и) местом(ами)",
            Name, ChildrenSeats);
            return s;
        }
    }
 ...
 : base(name)
    ...
    class ChildrenTransCom : TransportCompany
    {
        public ChildrenTransCom(string name)
        : base(name)
        { }
        public override TransportService Create(string n, int t)
        {
            return new TaxiForChildren(Name, t);
        }
    }
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            TransportCompany chTaxi = new ChildrenTransCom("Детское такси");
            compService = chTaxi.Create("Детское такси", 2);
            double distc = 20;
